...
static void world_partition_cell_add_entity(E02_Entity* entity, E02_WorldPartitionCell* cell)
{
...
static void collision_check_references(E02_GameState* state, E02_Entity** entity_refs, int entity_refs_count)
{
	for(int i = 0; i < entity_refs_count - 1; ++i)
	{
		E02_Entity* e1 = entity_refs[i];
...
static void game_init(E02_SDLContext* context, E02_GameState* state)
{
	...
	// world partitioning data allocation
	{
		...
		state->world_partition_cells = (E02_WorldPartitionCell*)SDL_calloc(state->world_partition_cells_count, sizeof(E02_WorldPartitionCell));

		// allocate actual array of references for the entities
		for(int i = 0; i < state->world_partition_cells_count; ++i)
		{
			E02_WorldPartitionCell* cell = &state->world_partition_cells[i];
			cell->entity_refs = (E02_Entity**)SDL_calloc(WORLD_PARTITION_CELL_MAX_ENTITY_COUNT, sizeof(E02_Entity*));
		}
	}
