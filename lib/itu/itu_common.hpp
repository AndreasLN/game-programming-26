...
// clamps each channel between 0 and 1
...
float fn_easing_linear(float t);
float fn_easing_constant_0(float t);
float fn_easing_constant_1(float t);
float fn_easing_out_quad (float t);
float fn_easing_out_cubic(float t);
float fn_easing_out_quart(float t);
float fn_easing_out_quint(float t);
float fn_easing_out_expo (float t);
float fn_easing_out_elastic (float t);
float fn_easing_out_bounce(float t);
float fn_easing_in_quad (float t);
float fn_easing_in_cubic(float t);
float fn_easing_in_quart(float t);
float fn_easing_in_quint(float t);
float fn_easing_in_expo (float t);
float fn_easing_in_elastic (float t);
float fn_easing_in_bounce(float t);

float easing(float t, EasingFunction fn);

#endif // ITU_COMMON_HPP

#if (defined ITU_COMMON_IMPLEMENTATION) || (defined ITU_UNITY_BUILD)
...
// wrapper for no easing (it allows us to have a nice common interface)
inline float fn_easing_linear(float t) { return t; }

// returns always 0 unless `t == 1`
inline float fn_easing_constant_0(float t) { return t == 1 ? 1 : 0; }

// returns always 1 unless `t == 0`
inline float fn_easing_constant_1(float t) { return t == 0 ? 0 : 1; }

inline float fn_easing_out_quad (float t) { return 1 - (1 - t) * (1 - t); }
inline float fn_easing_out_cubic(float t) { return 1 - SDL_powf(1 - t, 3); }
inline float fn_easing_out_quart(float t) { return 1 - SDL_powf(1 - t, 4); }
inline float fn_easing_out_quint(float t) { return 1 - SDL_powf(1 - t, 5); }
inline float fn_easing_out_expo (float t) { return t >= 1 ? 1 : 1 - SDL_powf(2, -10.0f * t); };
inline float fn_easing_out_elastic (float t)
{
	if(t <= 0) return 0;
	if(t >= 1) return 1;
	return SDL_powf(2.0f, -10.0f * t) * SDL_sinf((t * 10.0f - 0.75f) * (2.0f * PI) / 3.0f) + 1;
};
inline float fn_easing_out_bounce(float t)
{
...
inline float fn_easing_in_quad (float t) { return t * t; }
inline float fn_easing_in_cubic(float t) { return t * t * t; }
inline float fn_easing_in_quart(float t) { return t * t * t * t; }
inline float fn_easing_in_quint(float t) { return t * t * t * t * t; }
inline float fn_easing_in_expo (float t) { return t <= 0 ? 0 : SDL_powf(2, 10.0f * t - 10.0f); };
inline float fn_easing_in_elastic (float t)
{
	if(t <= 0) return 0;
	if(t >= 1) return 1;
	return -SDL_powf(2.0f, 10.0f * t - 10.0f) * SDL_sinf((t * 10.0f - 10.75f) * (2.0f * PI) / 3.0f);
};
inline float fn_easing_in_bounce(float t) { return 1 - fn_easing_out_bounce(1.0f - t); }

inline float easing(float t, EasingFunction fn)
{
	switch(fn)
	{
		...
		default:
		{
			SDL_LogWarn(0, "unknown easing function %d\n", fn);
			return t;
		}
...
#endif // (defined ITU_COMMON_IMPLEMENTATION) || (defined ITU_UNITY_BUILD)
